...
function parseStyles(styles) {
  ...
  return stylesStr;
	...
	/**
	 * 建立 container>playbutton>playButtonIcon, 并替换原来的 el
	 * @param w data-gifffer-width="250"
	 * @param h data-gifffer-height="100"
	 * @param el 图片元素
	 * @param altText
	 * @param opts
	 * @returns {{c: Element, p: Element}}
	 */
function createContainer(w, h, el, altText, opts) {
		console.log('createContainer...',new Date() - 0);
  var alt;
...
//class
  ...
  return { c: con, p: play };
	...
	/**
	 *  根据原图的宽高，计算比率，对属性中的%宽高进行转换
	 * @param el container
	 * @param w data-gifffer-width="250"
	 * @param h
	 * @param wOrig 原图片的宽
	 * @param hOrig
	 * @returns {{w: *, h: *}} 返回计算后的图片宽高
	 */
...
function process(el, gifs, options) {
  ...
  el.onload = function() {
  	// not support canvas
    if(!isC) return;
		console.log('el onload...',new Date() - 0);
    w = w || el.width;
    h = h || el.height;
